...
class OGame(object):
    ...
    def get_flying_fleets_info(self):
        ...
        for fleet in fleets:
            ...
            fleet_token.update({'origin': coords_origin})

            dest_coords = fleet.find('span',{
                'class': 'destinationCoords tooltip'}).text.strip()
            fleet_token.update({'destination': dest_coords})
            ...
            #fleet_td = fleet.findAll('td')
            #fleet_td = str(fleet_td)
            #fleet_td = remove_html_tags(fleet_td)
            print(fleet_detail)
            #fleet_td = re.search(r'[A-Za-z]+\s*[A-Za-z]+:|\d+',fleet_td)
